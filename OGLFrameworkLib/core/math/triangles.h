...
namespace cguMath {
    ...
    template<typename real> using TriangleProperties = std::tuple<glm::tvec4<real, glm::highp>, glm::tvec3<real, glm::highp>, glm::tvec3<real, glm::highp>, glm::tvec4<real, glm::highp>>;

    template<typename real> TriangleProperties<real> CalcTriangleProperties(const Tri3<real>& tri) {
        auto a = tri[1] - tri[0];
        auto b = tri[2] - tri[0];
    ...
    template<typename real> glm::tvec4<real, glm::highp> CalculateCircumsphere(const Tri3<real>& tri) {
        return std::get<0>(CalcTriangleProperties(tri));
    }

    template<typename real, typename GEN, typename DIST = std::uniform_real_distribution<real>> glm::tvec3<real, glm::highp> SampleTriangleBarycentric(GEN& randomGenerator)
    {
        DIST dist;
    ...
    template<typename real, typename GEN, typename DIST = std::uniform_real_distribution<real>> glm::tvec3<real, glm::highp> SampleTriangle(const Tri3<real>& tri, std::mt19937& randomGenerator)
    {
